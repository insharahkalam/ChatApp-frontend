...
export default function Sidebar({ selectedChat, setSelectedChat }) {
  ...
  const listItems = useMemo(() => {
    ...
    const chatItems = chats.map((c) => {
      ...
      return {
        ...
        lastMessage: c.latestMessage?.content || "No messages yet",
        lastMessageTime: c.latestMessage?.createdAt,
        lastMessageSender: c.latestMessage?.sender,
        unreadCount: c.unreadCount || 0,
      };
    });
  ...
  return (
    <div className="w-[380px] h-screen bg-[#111b21] flex flex-col border-r border-black/40 relative">
      {/* Header */}
      <div className="px-4 py-3 bg-[#202c33] flex justify-between items-center shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#00a884] to-[#008069] flex items-center justify-center font-bold text-white">
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="relative">
            <IconButton title="Menu" onClick={() => setMenuOpen((v) => !v)}>
              <MoreVertical size={20} strokeWidth={1.8} />
            </IconButton>
            {menuOpen && (
              <div
                className="absolute right-0 top-11 w-44 bg-[#233138] rounded-md shadow-xl py-1.5 z-20 overflow-hidden"
                onMouseLeave={() => setMenuOpen(false)}
              >
                <button
                  onClick={logout}
                  className="w-full flex items-center gap-2.5 px-4 py-2.5 text-[14px] text-[#e9edef] hover:bg-[#182229] transition"
                >
                  <LogOut size={16} />
                  Log out
                </button>
        ...
        {showingSearch
          ...
          : listItems.map((item) => (
            <UserRow
              ...
              active={selectedChat?._id === item.chat?._id}
              unreadCount={item.unreadCount}
              isOwnLastMessage={item.lastMessageSender === user._id}
              onClick={() =>
                item.type === "chat" ? setSelectedChat(item.chat) : accessChat(item.userId)
              }
            />
...
function IconButton({ children, title, onClick }) {
  return (
    <button
      title={title}
      onClick={onClick}
      className="w-9 h-9 rounded-full flex items-center justify-center text-[#aebac1] hover:bg-white/10 hover:text-white transition"
    >
      {children}
    </button>
  );
...
function TabButton({ label, count, icon, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`px-3 py-1.5 rounded-full text-[13px] font-medium transition flex items-center gap-1.5 ${active ? "bg-[#00a884] text-[#0b141a]" : "bg-[#202c33] text-[#e9edef] hover:bg-[#2a3942]"
        }`}
    >
      {icon}
      {label}
      {!!count && (
        <span className={`text-[11px] ${active ? "text-[#0b141a]/70" : "text-[#8696a0]"}`}>
          {count}
        </span>
      )}
    </button>
...
function UserRow({
  ...
  return (
    ...
    >
      <div className="relative shrink-0">
        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#00a884] to-[#008069] text-white flex items-center justify-center font-bold text-lg">
          {avatarLetter ? avatarLetter.toUpperCase() : <CircleUserRound size={22} />}
        </div>
        {isOnline && (
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex justify-between items-center mt-0.5">
          <p
            className={`text-[13px] truncate flex items-center gap-1 ${isOnline ? "text-[#00a884]" : unreadCount > 0 ? "text-[#e9edef]" : "text-[#8696a0]"
              }`}
          >
            {isOwnLastMessage && !isOnline && (
...
function EmptyState({ text }) {
  return (
    <div className="flex flex-col items-center justify-center mt-16 gap-2 text-[#8696a0]">
      <Search size={28} />
